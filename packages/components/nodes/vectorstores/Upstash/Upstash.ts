...
class Upstash_VectorStores implements INode {
    ...
    async init(nodeData: INodeData, _: string, options: ICommonObject): Promise<any> {
        ...
        if (isFileUploadEnabled && options.chatId) {
            if (upstashMetadataFilter) obj.filter += ` OR ${SynapseFlow_CHATID} = "${options.chatId}" OR HAS NOT FIELD ${SynapseFlow_CHATID}`
            else obj.filter = `${SynapseFlow_CHATID} = "${options.chatId}" OR HAS NOT FIELD ${SynapseFlow_CHATID}`
        }
