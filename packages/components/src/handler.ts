...
class ExtendedLunaryHandler extends LunaryHandler {
    ...
    constructor({ SynapseFlowOptions, ...options }: any) {
        super(options)
        this.appDataSource = SynapseFlowOptions.appDataSource
        this.databaseEntities = SynapseFlowOptions.databaseEntities
        this.chatId = SynapseFlowOptions.chatId
        this.apiMessageId = SynapseFlowOptions.apiMessageId
    }
...
export class AnalyticHandler {
    ...
    async onChainStart(name: string, input: string, parentIds?: ICommonObject) {
        ...
        if (Object.prototype.hasOwnProperty.call(this.handlers, 'arize')) {
            ...
            if (!parentIds || !Object.keys(parentIds).length) {
                rootSpan = tracer ? tracer.startSpan('SynapseFlow') : undefined
                if (rootSpan) {
