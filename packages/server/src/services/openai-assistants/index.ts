...
import { getFileFromUpload, removeSpecificFileFromUpload } from 'SynapseFlow-components'
...
const getAllOpenaiAssistants = async (credentialId: string): Promise<any> => {
    try {
        ...
        if (!credential) {
            throw new InternalSynapseFlowError(StatusCodes.NOT_FOUND, `Credential ${credentialId} not found in the database!`)
        }
