import { handleEscapeCharacters, ICommonObject } from 'SynapseFlow-components'
    ...
    try {
        ...
        if (Object.prototype.hasOwnProperty.call(componentNodes, 'customFunction')) {
            try {
                ...
                throw new InternalSynapseFlowError(StatusCodes.INTERNAL_SERVER_ERROR, `Error running custom function: ${error}`)
            }
        } else {
            throw new InternalSynapseFlowError(StatusCodes.NOT_FOUND, `Node customFunction not found`)
        }
    } catch (error) {
