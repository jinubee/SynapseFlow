...
export class Prometheus implements IMetricsProvider {
    ...
    async initializeCounters(): Promise<void> {
        const serviceName: string = process.env.METRICS_SERVICE_NAME || 'SynapseFlowAI'
        ...
        try {
            const versionGaugeCounter = new promClient.Gauge({
                name: 'SynapseFlow_version_info',
                help: 'SynapseFlow version info.',
                labelNames: ['version'],
                registers: [this.register] // Explicitly set the registry
            })
            ...
            this.counters.set('SynapseFlow_version', versionGaugeCounter)
        } catch (error) {
            // If metric already exists, get it from the registry
            const existingMetric = this.register.getSingleMetric('SynapseFlow_version')
            if (existingMetric) {
                this.counters.set('SynapseFlow_version', existingMetric as promClient.Gauge<string>)
            }
        }
    ...
    public incrementCounter(counter: SynapseFlow_METRIC_COUNTERS, payload: any) {
    ...
    private registerMetrics() {
        if (process.env.METRICS_INCLUDE_NODE_METRICS !== 'false') {
            ...
            promClient.collectDefaultMetrics({
                ...
                prefix: 'SynapseFlow_' // Add a prefix to avoid conflicts
            })
        }
