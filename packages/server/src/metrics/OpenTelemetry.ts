import { SynapseFlow_METRIC_COUNTERS, IMetricsProvider } from '../Interface.Metrics'
...
import { getVersion } from 'SynapseFlow-components'
...
export class OpenTelemetry implements IMetricsProvider {
    ...
    async initializeCounters(): Promise<void> {
        try {
            ...
            const SynapseFlowVersion = await getVersion()

            this.resource = new Resource({
                [ATTR_SERVICE_NAME]: process.env.METRICS_SERVICE_NAME || 'SynapseFlowAI',
                [ATTR_SERVICE_VERSION]: SynapseFlowVersion.version // Version as a label
            })
            ...
            const meter = this.meterProvider.getMeter('SynapseFlow-metrics')
            // look at the SynapseFlow_COUNTER enum in Interface.Metrics.ts and get all values
            // for each counter in the enum, create a new promClient.Counter and add it to the registry
            const enumEntries = Object.entries(SynapseFlow_METRIC_COUNTERS)
            ...
            try {
                ...
                if (!createdMetrics.has('SynapseFlow_version')) {
                    const versionGuage = meter.createGauge('SynapseFlow_version', {
                        description: 'SynapseFlow version'
                    })
                    // remove the last dot from the version string, e.g. 2.1.3 -> 2.13 (gauge needs a number - float)
                    const formattedVersion = SynapseFlowVersion.version.replace(/\.(\d+)$/, '$1')
                    versionGuage.record(parseFloat(formattedVersion))
                    createdMetrics.set('SynapseFlow_version', true)
                }
            } catch (error) {
