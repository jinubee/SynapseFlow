/**
 * Copyright (c) 2023-present SynapseFlowAI, Inc.
 *
 * The Enterprise and Cloud versions of SynapseFlow are licensed under the [Commercial License](https://github.com/SynapseFlowAI/SynapseFlow/tree/main/packages/server/src/enterprise/LICENSE.md).
 * Unauthorized copying, modification, distribution, or use of the Enterprise and Cloud versions is strictly prohibited without a valid license agreement from SynapseFlowAI, Inc.
 *
...
export class IdentityManager {
    ...
    private _validateLicenseKey = async () => {
        ...
        const SynapseFlow_EE_LICENSE_KEY = process.env.SynapseFlow_EE_LICENSE_KEY

        // First check if license key is missing
        if (!SynapseFlow_EE_LICENSE_KEY) {
        ...
        try {
            if (process.env.OFFLINE === 'true') {
                const decodedLicense = this._offlineVerifyLicense(SynapseFlow_EE_LICENSE_KEY)
                ...
                try {
                    const response = await axios.post(`${LICENSE_URL}/enterprise/verify`, { license: SynapseFlow_EE_LICENSE_KEY })
                    ...
                    else throw new InternalSynapseFlowError(StatusCodes.INTERNAL_SERVER_ERROR, GeneralErrorMessage.UNHANDLED_EDGE_CASE)
                } catch (error) {
                    console.error('Error verifying license key:', error)
                    this.licenseValid = false
                    this.currentInstancePlatform = Platform.ENTERPRISE
    ...
    public async updateSubscriptionPlan(req: Request, subscriptionId: string, newPlanId: string, prorationDate: number) {
        ...
        if (!req.user) {
            throw new InternalSynapseFlowError(StatusCodes.UNAUTHORIZED, GeneralErrorMessage.UNAUTHORIZED)
        }
        ...
        if (success) {
            ...
            req.session.save((err) => {
                if (err) throw new InternalSynapseFlowError(StatusCodes.BAD_REQUEST, GeneralErrorMessage.UNHANDLED_EDGE_CASE)
            })
