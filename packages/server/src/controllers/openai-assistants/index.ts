...
import { streamStorageFile } from 'SynapseFlow-components'
...
const getFileFromAssistant = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (!fileStream) throw new InternalSynapseFlowError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: getFileFromAssistant`)
